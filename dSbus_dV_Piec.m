function [dSbus_dVm, dSbus_dVa] = dSbus_dV_Piec(Ybus, V, IExt)
...
if issparse(Ybus)           %% sparse version (if Ybus is sparse)
    ...
    diagIExt    = sparse(1:n, 1:n, IExt, n,n);
else                        %% dense version
    diagV       = diag(V);
    diagIbus    = diag(Ibus);
...
dSbus_dVm = diagV * conj(Ybus * diagVnorm) + conj(diagIbus+diagIExt) * diagVnorm;
dSbus_dVa = 1j * diagV * conj(diagIbus+diagIExt - Ybus * diagV);
